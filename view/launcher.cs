...
namespace PharmacySystem.view
{
...

    {
        ...
        private List<Sale_report> sale_Reports;
        ...
        */
        ...
        private void SearchMed_TextChanged(object sender, EventArgs e)
        {
            ...
            using (var db = new DataBaseConnection())
            {
                medicines = db.medicines
                    .ToList();
                String searchText = this.searchMed.Text.ToLower();
                
                foreach(var med in medicines)
                {
                    if (//this.searchMed.Text.Contains (med.name)
                        med != null &&((med?.name ?? "NullName").ToLower().Contains(searchText)
                        || ((med?.generic_name ?? "NullGeneric").ToLower().Contains(searchText))
                        || ((med?.category?.name ?? "NullCategory").ToLower().Contains(searchText))
                        || ((med?.Type?.name ?? "NullType").ToLower().Contains(searchText))
                        || (med?.company?.name ?? "NullCompany").ToLower().Contains(searchText)))
        ...
        private void BtnBuy_Click(object sender, EventArgs e)
        {
            ...
            sale_Reports = new List<Sale_report>();
            ...
            foreach (Control ctrl in flowLayoutPanel1.Controls)
            {
                if (ctrl is Panel itemPanel)
                {
                    ...
                    if (nameTextBox != null && countTextBox != null && price != null)
                    {
                        ...
                        // Find the medicine from the original list
                        var selectedMed = medicines.FirstOrDefault(m => m.medicine_id == id);
                        if (selectedMed != null)
                        {
                            var report = new Sale_report
                            {
                                medicine_id = id,
                                count = count,
                                sale_unit_price =  (double)price_unit
                            };
            ...
            if (sale_Reports.Any())
            {
                SalesReportView sales = new SalesReportView(sale_Reports)
                {
                    Visible = true
                };
            }
